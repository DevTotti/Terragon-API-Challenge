...
def temperature():
    ...
    newDataBase = "terrisstifwefuwinwwegoiw"
    if conn.exists(newDataBase) == 1:
    	...
    	if str(IP_Address) in newData:
    		...
    		print("Data do not exist in database")
    		json_object = cleanJson(IP_Address)
    		json_object = json.dumps(json_object).replace("null",'"NIL"')
...
def cleanJson(IP_Address):
	r = requests.get('http://api.ipstack.com/'+IP_Address+'?access_key=024e3f392effb6c46a4003ad68617a4f')
	json_object = r.json()
	del json_object["location"]
	delete = ("zip","type","continent_code","country_code","region_code")
